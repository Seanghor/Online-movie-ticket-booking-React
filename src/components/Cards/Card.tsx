...
export const Card: React.FunctionComponent<MovieCardProps> = (props: MovieCardProps) => {
    const { image, title, opening_date, onClick, isLoading } = props;
    return (
        <>
            {
                isLoading ? (<SkeletonMoviePage />) : (<div onClick={() => onClick()} className="shadow-cyan-500/50 h-[350px] hover:border-2 border-cyan-100 flex-shrink-0 m-4 w-56  relative overflow-hidden bg-slate-950 rounded-lg max-w-md shadow-2xl">
